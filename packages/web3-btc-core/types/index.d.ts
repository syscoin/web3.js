...
export interface PromiEvent<T> extends Promise<T> {
    ...
    once(
        type: 'confirmation',
        handler: (confNumber: number, transaction: string) => void
    ): PromiEvent<T>;

    once(type: 'error', handler: (error: Error) => void): PromiEvent<T>;

    once(
        type: 'error' | 'confirmation' | 'transactionHash',
...
export class AccountsBase {
    ...
    signTransaction(
        transaction: string,
        privateKey: string,
        callback?: (error: Error, signedTransaction: string) => void
    ): Promise<string>;
...
export interface Account {
    ...
    signTransaction: (
        transaction: string,
        callback?: (signTransaction: string) => void
    ) => Promise<string>;
...
export type BlockNumber =  number | BN | BigNumber;
